...
public Duration calculateWorkingHours(final LocalDateTime start, final LocalDateTime end) {
        ...
        while (current.isBefore(end)) {
            if (isWeekday(current)) {
                // Adjust start time to work start if it's before work hours
                LocalDateTime workStart = current.toLocalDate().atTime(WORK_START_TIME);
                LocalDateTime effectiveStart = current.isBefore(workStart) ? workStart : current;
                ...
                if (effectiveStart.isBefore(actualEnd)) {
                    totalWorkingDuration = totalWorkingDuration.plus(Duration.between(effectiveStart, actualEnd));
                }
            }
